...
export const BirdImage: React.FC<BirdImageProps> = ({
  ...
  const getGradient = (birdName: string) => {
    ...
    if (lower.includes('merle') || lower.includes('grive')) return 'from-slate-800 via-zinc-900 to-amber-500';
    ...
    if (lower.includes('martinet') || lower.includes('hirondelle')) return 'from-sky-900 via-slate-800 to-amber-900';
    if (lower.includes('héron') || lower.includes('aigrette') || lower.includes('spatule')) return 'from-slate-400 via-sky-600 to-indigo-900';
    if (lower.includes('chardonneret')) return 'from-red-500 via-yellow-400 to-slate-900';
    if (lower.includes('faucon') || lower.includes('épervier') || lower.includes('buse') || lower.includes('balbuzard')) return 'from-amber-700 via-yellow-700 to-stone-800';
    if (lower.includes('canard') || lower.includes('cygne') || lower.includes('foulque') || lower.includes('grèbe')) return 'from-emerald-700 via-teal-800 to-amber-600';
    ...
    if (lower.includes('cigogne') || lower.includes('ibis')) return 'from-red-950 via-slate-900 to-amber-900';
    ...
    if (lower.includes('fou') || lower.includes('macareux') || lower.includes('goéland') || lower.includes('sterne') || lower.includes('puffin')) return 'from-sky-600 via-teal-700 to-indigo-900';
    if (lower.includes('cormoran') || lower.includes('huîtrier') || lower.includes('gravelot')) return 'from-slate-800 via-cyan-900 to-teal-900';
